...
public class DBHelper extends SQLiteOpenHelper {
    ...
    // Method to update username, users table, and daily_weight table to new username
    public boolean updateUsername(String username, String newUsername) {
        ...
        try {
            ...
            int rows1 = db.update(TABLE_USERS, cv, "username=?",
                    new String[]{ username });

            // Update daily_weight table foreign keys
            ContentValues cv2 = new ContentValues();
            cv2.put(COL_USER_FK, newUsername);
            int rows2 = db.update(TABLE_WEIGHT, cv2, COL_USER_FK + "=?",
                    new String[]{ username });
    ...
    // Method to update password associated with username
    public boolean updatePassword(String username, String newPassword) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues cv = new ContentValues();
        cv.put("password", newPassword);

        int rows = db.update("users", cv, "username=?",
                new String[]{ username });
        return rows > 0;
    }
    ...
    // Acquires all daily weight entries as a list of DataItem objects for a specific user
